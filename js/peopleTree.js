...
var isInPoly = function (pt) {
    for (var i = 0; i < safeArea.getSource().getFeatures().length; i++) {
        //遍历安全区是否相交
        var poly = polygon(safeArea.getSource().getFeatures()[i].getGeometry().getCoordinates()[0]);
...
var renderWarning = function (vector, pointFeature, isSafe) {
    ...
    fetch('https://api2.bmob.cn/1/classes/position', {
        ...
        for (var i = 0; i < result.results.length; i++) {
            if (!vectorPhonePeoNames.includes(result.results[i].name) || vectorPhonePeoNames.length == 0) {
                ...
                var pointFeaturePhonePeo = new Feature();
...
var point1 = new Point(fromLonLat([116.20063511, 39.85862116]));
var point3 = new Point(fromLonLat([116.23814219, 39.82811664]));
//1
...
//2
var point2 = new Point(fromLonLat([116.21810105, 39.83500744]));
...
function runPeopleMove(peoplePostionPath, vector, i, tag) {
    ...
    if (peoplePostionPath[i] == undefined) {
        ...
        if (pointFeature == undefined)
            return;
        pointFeature.getGeometry().setCoordinates(fromLonLat([x, y]));
